...
public class BotFilter
{
    ...
    public void disable()
    {
        ...
        for ( Connector connector : connectedUsersSet.values() )
        {
            ...
            {
                connector.getUserConnection().disconnect( "§c[BotFilter] §aRecarga del filtro" );
            }
    ...
    private void checkForUpdates(boolean startup)
    {
        ...
        try
        {
            logger.log( Level.INFO, "[BotFilter] Verificando si hay actualizaciones" );
                    ...
                    new InputStreamReader( conn.getInputStream() ) ) )
            {
                if ( !in.readLine().trim().equalsIgnoreCase( Settings.IMP.BOT_FILTER_VERSION ) )
                {
                    ...
                    logger.log( Level.INFO, "§c[BotFilter] §a¡Se ha encontrado una nueva versión!" );
                    logger.log( Level.INFO, "§c[BotFilter] §a¡Por favor, actualice!" );
                ...
                {
                    logger.log( Level.INFO, "[BotFilter] ¡No se encontraron actualizaciones!" );
                }
            }
        } catch ( IOException | InterruptedException ex )
        {
            logger.log( Level.WARNING, "[BotFilter] No se pudo verificar la actualización", ex );
        }
    }
