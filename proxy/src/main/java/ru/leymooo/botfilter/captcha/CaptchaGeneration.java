...
public class CaptchaGeneration
{
    ...
    public void generateImages()
    {
        ...
        {
            BungeeCord.getInstance().getLogger().log( Level.INFO, "[BotFilter] Generando captcha [{0}/900]", 900 - ex.getQueue().size() - ex.getActiveCount() );
            try
            {
                Thread.sleep( 1000L );
            } catch ( InterruptedException ex1 )
            {
                BungeeCord.getInstance().getLogger().log( Level.WARNING, "[BotFilter] No se pudo generar la captcha. Apagando Bungee", ex1 );
                System.exit( 0 );
                return;
            }
        }
        ...
        System.gc();
        BungeeCord.getInstance().getLogger().log( Level.INFO, "[BotFilter] La captcha se ha generado en {0} ms", System.currentTimeMillis() - start );
    }
